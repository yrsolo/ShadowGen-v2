...
- the worker reuses one ML-core adapter per effective ML URL, so capability probing is cached across warm jobs until `CAPABILITIES_REFRESH_INTERVAL_SEC` expires; the default is 300 seconds, and effective ML URL changes still create a fresh adapter
