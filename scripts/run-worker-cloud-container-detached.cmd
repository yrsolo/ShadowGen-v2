...
echo [ShadowGen] Starting detached worker container against cloud services...
echo [ShadowGen] This is the preferred always-on Docker mode for a permanent worker host.

if not exist ".env.shadowgen" (
  echo [ShadowGen] .env.shadowgen is missing.
  echo [ShadowGen] Copy .env.shadowgen.example to .env.shadowgen and fill it first.
  exit /b 1
)
